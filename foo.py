...
\x0c
    python3 emergency_git_server.py [DOCROOT]
        ...
        DOCROOT
            ...
            in dot git, e.g., /var/www/html/repository/foo.git
    ...
    Environment options:
        ...
        _ALLOW_CREATION
            ...
            As with existing repository, these must end in ".git".  Note: HEAD is
        ...
        if not is_ghb_bound(self.command, self.path):
            if self.command == "POST":
                if self.path.rstrip("/").endswith(".git"):
                    ...
                    msg = "Non-git POST only allowed when creating new repository"
                    self.send_error(HTTPStatus.METHOD_NOT_ALLOWED, msg)
                return False
            return True
